...
.section .itcm,"ax",%progbits
	...
	@; _gs_num2str_dec: permite convertir un nÃºmero natural de 32 bits a su
	@;					representaciÃ³n en decimal, dentro de un string acabado
	@;					en cero ('\0');
	@;ParÃ¡metros
	@; R0: char * numstr,
...
_gs_num2str_dec:
	push {r1-r8, lr}
	cmp r1, #1
	bhi .Ln1s_cont1			@; verificar si hay espacio para centinela + 1 dÃ­gito
	mov r0, #-1
	b .Ln1s_fin				@; retornar con cÃ³digo de error en R0
.Ln1s_cont1:
	mov r6, r0				@; R6 = puntero a string de resultado
	mov r7, r1				@; R7 = Ã­ndice carÃ¡cter (inicialmente es la longitud del string)
	mov r8, r2				@; R8 = nÃºmero a transcribir
	mov r3, #0
	sub r7, #1
...
.Ln1s_while:	
	...
	mov r2, sp				@; pasar direcciÃ³n para albergar el cociente
	add r3, sp, #4			@; pasar direcciÃ³n para albergar el resto
	bl _ga_divmod
	pop {r4-r5}				@; R4 = cociente, R5 = resto
	add r5, #48				@; aÃ±adir base de cÃ³digos ASCII para dÃ­gitos numÃ©ricos
	sub r7, #1
	strb r5, [r6, r7]		@; almacenar cÃ³digo ASCII en vector
	mov r8, r4				@; actualizar valor del nÃºmero a convertir
	cmp r8, #0				@; repetir mientras el nÃºmero sea diferente de 0
	bne .Ln1s_while
.Ln1s_pad:
	...
	mov r3, #' '
	sub r7, #1
	strb r3, [r6, r7]		@; almacenar cÃ³digo ASCII ' ' en vector
	b .Ln1s_pad
.Ln1s_cont2:
	mov r0, r8				@; esto indicarÃ¡ si el numero se ha podido codificar
.Ln1s_fin:					@; completamente en el string (si R0 = 0)
	...
	@; _gs_num2str_hex: permite convertir un nÃºmero natural de 32 bits a su
	@;					representaciÃ³n en hexadecimal, dentro de un string
	@;					acabado en cero ('\0');
	@;ParÃ¡metros
	@; R0: char * numstr,
	@; R1: unsigned int length,
...
_gs_num2str_hex:
	push {r1-r4, lr}
	cmp r1, #1
	bhi .Ln2s_cont1			@; verificar si hay espacio para centinela + 1 dÃ­gito
	mov r0, #-1
	b .Ln2s_fin				@; retornar con cÃ³digo de error en R0
.Ln2s_cont1:
	mov r3, #0
	sub r1, #1
	strb r3, [r0, r1]		@; guardar final de string (0) en Ãºltima posiciÃ³n
.Ln2s_while:	
	cmp r1, #0				@; repetir mientras quede espacio en el string
	beq .Ln2s_cont2
	and r4, r2, #0x0F		@; obtener el dÃ­gito hexa de menos peso
	cmp r4, #10				@; si dÃ­gito hexa menor que 10, saltar a tratamiento
	blo .Ln2s_dec			@; de dÃ­gitos decimales
	add r4, #7				@; ajuste para letras 'A'-'F' (65 - 10 - 48)
.Ln2s_dec:
	add r4, #48				@; aÃ±adir base de cÃ³digos ASCII para nÃºmeros
	sub r1, #1
	strb r4, [r0, r1]		@; almacenar cÃ³digo ASCII en vector
	mov r2, r2, lsr #4		@; actualizar valor del nÃºmero a convertir
	cmp r2, #0				@; repetir mientras el nÃºmero sea diferente de 0
	bne .Ln2s_while
.Ln2s_pad:
	...
	mov r3, #'0'
	sub r1, #1
	strb r3, [r0, r1]		@; almacenar cÃ³digo ASCII '0' en vector
	b .Ln2s_pad
.Ln2s_cont2:
	mov r0, r2				@; esto indicarÃ¡ si el numero se ha podido codificar
.Ln2s_fin:					@; completamente en el string (si R0 = 0)
	...
	.global _gs_copiaMem
	@; Rutina para copiar un bloque de memoria desde una direcciÃ³n fuente a 
	@; otra direcciÃ³n destino, el nÃºmero de bytes indicado:
	@;ParÃ¡metros:
	@; R0: direcciÃ³n fuente (debe ser mÃºltiplo de 4)
	@; R1: direcciÃ³n destino (debe ser mÃºltiplo de 4)
	@; R2: nÃºmero de bytes a copiar
_gs_copiaMem:
	push {r0-r12, lr}
...
.LcopMem_fin:
	...
	@;	caracteres y poner el puntero de lÃ­nea actual al principio (fila 0)
	@;ParÃ¡metros:
	@;	R0: Ã­ndice de ventana (0..15)
	@;	R1: nÃºmero de ventanas (0 -> 4 ventanas, 1 -> 16 ventanas)
	@;Â¡ATENCIÃ“N! Para que esta rutina funcione correctamente, es necesario
	@; haber inicializado el procesador grÃ¡fico principal correctamente
	@; (baldosas, paleta, mapa de baldosas de fondo 2 con las dimensiones
	@;  adecuadas, etc.).
_gs_borrarVentana:
	push {r0-r5, lr}
								@; (para procesador grÃ¡fico principal)
...
.LborrarV_for2:					@; bucle para recorrer las casillas de la fila
	...
	.global _gs_iniGrafB
	@; Inicializa el procesador grÃ¡fico B (sub) para GARLIC 2.0:
	@; memoria de vÃ­deo en el banco de vÃ­deo-RAM C (0x06200000);
	@; fondo 0 de texto (32x32) caracteres, mapa en la base 0;
	@; 128 baldosas (8 bits por pÃ­xel) en la base 1, y dibujos de
	@; baldosas repetidos 3 veces, una por cada color (512 baldosas);
	@; 96 baldosas correspondientes a los grÃ¡ficos de ocupaciÃ³n de memoria,
...
_gs_iniGrafB:
 ...
 @; videoSetModeSub(MODE_5_2D);
	...
	mov r1, #0x00010000			@; R1 = modo grÃ¡fico 0, pantalla activada
	orr r1, #0x00000100			@;		fondo 0 activado (texto)
	str r1, [r0]				@; fijar modo de vÃ­deo procesador grÃ¡fico B

 @;	vramSetBankC(VRAM_C_SUB_BG_0x06200000);
	ldr r0, =0x04000242			@; R0 = dir. registro control banco vÃ­deo-RAM C
	mov r1, #0x84				@; R1 = memoria fondo para procesador grÃ¡fico B
 ...
 @; bg0B = bgInitSub(0, BgType_Text8bpp, BgSize_T_256x256, map_base, tile_base);
	...
	mov r1, #0x87				@; R1 -> priority = 0 (bits 1..0 = 00)
 ...
 @;	newtile_base = (unsigned int *) (0x06204000 + 512 * 64);
	add r1, r3, #0x8000			@; R1 = base de baldosas para gestiÃ³n de memoria
 @;	mapMem_base = BG_MAP_RAM_SUB(0) + 32*21;
	add r3, r4, #0x540			@; R3 = posiciÃ³n fila 21 del mapa de caracteres
	mov r2, #64					@; R2 = longitud del contenido de una baldosa
	mov r4, #0					@; R4 es Ã­ndice i
	mov r5, #512				@; R5 es R4 + 512
 @;	for (i = 0; i < 96; i++)
.LiniGrafB_for1:
 @;	{	dmaCopy(tile119_base, newtile_base, 64);
	bl _gs_copiaMem				@; copiar pÃ­xeles de la nueva baldosa
 @;		newtile_base += 16;					// sumar 64 bytes (16 words)
	add r1, #64					@; actualizar puntero a otra nueva baldosa
 @;		mapMem_base[i] = 512+i;
	strh r5, [r3]				@; guardar cÃ³digo baldosa en mapa de caracteres
	add r4, #1					@; i++;
...
.LiniGrafB_for2:
 @;	{
	bl _gs_copiaMem				@; copiar pÃ­xeles de todas las baldosas

	add r9, r8, r4, lsl #4		@; R9 = color de paleta
	mov r5, #0					@; R5 es Ã­ndice j
 @; for (j = 0; j < 128; j++)
.LiniGrafB_for3:
 @; {
	ldr r3, [r1, r5]			@; R3 = *(tile_ptr + j)
	mov r7, #0					@; R7 es newColors
	mov r6, #0					@; R6 es Ã­ndice k
 @; for (k = 0; k < 4; k++)
.LiniGrafB_for4:
 @; {
...
.LiniGrafB_noColor:
	...
	mov r7, r7, lsr #8			@; desplazar patrÃ³n del nuevo color
	mov r3, r3, lsr #8			@; y desplazar el patrÃ³n del color actual
	b .LiniGrafB_for4
.LiniGrafB_fifor4:
 @; }
	str r7, [r1, r5]			@; actualizar el nuevo patrÃ³n de color
	add r5, #4
	cmp r5, r2
	blo .LiniGrafB_for3
 @; }	
 ...
 @;	}
	...
	@; de la posiciÃ³n indicada por parÃ¡metros (fil, col), con el color especifi-
	@; cado, en la pantalla secundaria;
	@;ParÃ¡metros
	@;	R0: direcciÃ³n inicial del string
	@;	R1: fila inicial
	@;	R2: columna inicial
	@;	R3: color del texto
_gs_escribirStringSub:
	push {r0-r4, lr}
								@; (para procesador grÃ¡fico secundario)
...
.LescStrSub_0:
	...
	and r3, #0x03				@; R3 = color (sÃ³lo admite valores 0..3)
...
.LescStrSub_while:
	...
	blo .LescStrSub_fiwhile	 	@; o cÃ³digo ASCII no imprimible (R1 < 32)
	sub r1, #32					@; restar base cÃ³digos ASCII imprimibles
	...
	add r2, #1					@; avanzar Ã­ndice de columna
	cmp r2, #32
	blo .LescStrSub_while		@; seguir mientras no llegue a Ãºltima columna
.LescStrSub_fiwhile:
	pop {r0-r4, pc}
...
_gs_dibujarTabla:
	...
	mov r3, #0
	mov r4, #0					@; R4 es Ã­ndice del bucle
.LdibTab_for1:
...
.LdibTab_for2:
	...
	@;_gg_escribirStringSub((char *) tabla[5], 20, 0, 0);	// Ãºltima linea
	add r0, #33
	mov r1, #20
	mov r3, #0
	bl _gs_escribirStringSub
