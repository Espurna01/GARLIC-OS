		...
		if (buffer != NULL){
			...
			//3. acceder a la cabecera ELF para obtener el offset y tamaño de tabla de segmentos
				...
				if (tabSeg_entradas != 0){									
					...
					// hacer una comprobación, si son dos trataremos primero codigo y luego datos
					...
					if (tabSeg_entradas == 1){
						if (tabla_segm_code == 1){
							...
							if (reservaMemCode!=0){
								_gs_copiaMem((const void *) &buffer[prog_offset], (unsigned int *) reservaMemCode, (unsigned int) prog_size);		//dirección fuente, destino y tamaño del segmento
								
								//7. Efectuar la reubicacion de las posiciones sensibles (La otra fx)
								_gm_reubicar( buffer, (unsigned int)prog_segmentoInicial, (unsigned int *) reservaMemCode ,0XFFFFFFFF,0);		//buffer del fichero, dirección donde está el segmento, destino en memoria, inicio segmento de datos
							...
							if((tabla_segm_code == 1) && (tabla_seg_dat_type == 1)){
								...
								if (reservaMemDatos!=0){
									...
									if (reservaMemCode!=0){
										...
										_gs_copiaMem((const void *) &buffer[prog_offset], (unsigned int *) reservaMemCode, (unsigned int) prog_size);		//dirección fuente, destino y tamaño del segmento
										_gs_copiaMem((const void *) &buffer[tabla_segm_data->p_offset], (unsigned int *) reservaMemDatos, (unsigned int) tabla_segm_data->p_filesz);		//dirección fuente, destino y tamaño del segmento
										
										//7. Efectuar la reubicacion de las posiciones sensibles (La otra fx)
										_gm_reubicar(buffer, (unsigned int)prog_segmentoInicial, (unsigned int *) reservaMemCode, tabla_segm_data->p_paddr, (unsigned int *) reservaMemDatos);		//buffer del fichero, dirección donde está el segmento, destino en memoria, inicio segmento de datos
